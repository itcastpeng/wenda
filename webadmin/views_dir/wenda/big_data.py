...
def big_data(request):
    ...
    if "type" in request.GET and request.GET["type"] == "ajax_json":
        ...
        # 排序
        column_list = ["id", "username", "status", "balance", "xie_wenda_money", "fa_wenda_money", "create_date", "last_login_date", "oper", "role_names"]
        ...
        for index, obj in enumerate(user_profile_objs[start: (start + length)], start=1):
            ...
            result_data["data"].append([index, obj.username, obj.role.name, status, obj.balance, obj.xie_wenda_money, obj.fa_wenda_money, create_date, last_login_date, oper])
