...
def my_client(request):
    ...
    if "type" in request.GET and request.GET["type"] == "ajax_json":
        ...
        for index, obj in enumerate(clinet_date_objs[start: (start + length)], start=1):
            ...
            beizhu = "<a href='marker_client/{user_id}/' data-toggle='modal' data-target='#exampleFormModal'>备注</a>".format(
                user_id=user_id)
...
def my_client_oper(request, oper_type, o_id):
    ...
    if request.method == "POST":
        ...
        if oper_type =='marker_client':
            ...
            if objs:
                if objs[0].remark_beizhu:
                    print('objs[0].remark_beizhu =========== >',objs[0].remark_beizhu)
        ...
        # 查看日志
            ...
            #     guishu_user_id=o_id
            # )
            objs = models.My_Client_User_Log.objects.filter(guishu_user=o_id)
            result_data = ''
            tr_html = ''
            result_data = """
            ...
            for index, obj in enumerate(objs):
                ...
                tr_html += """
                     ...
                     <td>
                      ...
                      """.format(datetime=date_time,event=event,guishu=guishu)
