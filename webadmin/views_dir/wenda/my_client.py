...
def my_client(request):
    ...
    if "type" in request.GET and request.GET["type"] == "ajax_json":
        ...
        if role_id in [1,7]:
            ...
            clinet_date_objs = models.UserProfile.objects.filter(q).filter(role_id__in=[5, 15],is_delete=False)
            ...
            clinet_date_objs = models.UserProfile.objects.filter(q).filter(xiaoshou_id=obj_user_id).filter(role_id__in=[5, 15],is_delete=False)
        result_data = {
