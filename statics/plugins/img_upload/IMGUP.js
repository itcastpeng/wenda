...
var IMG_AJAXPATH = "/img_upLoad/";//异步传输服务端位置


var UP_IMGCOUNT = $("#div_imglook .lookimg").length;//上传图片张数记录

//打开文件选择对话框
$("#div_imgfile").unbind().click(function () {
    if ($("#div_imglook .lookimg").length >= IMG_MAXCOUNT) {
        toastr['error']("最多上传" + IMG_MAXCOUNT + "张图片");
        return;
    }
    ...
    if ($(".imgfile").length <= $(".lookimg").length) {//个数不足则新创建对象
        ...
        _CRE_FILE.setAttribute("class", "imgfile hidden");
...
$(".imgfile").die().live("change", function () {
...
//删除按钮移入移出效果
$("#div_imglook .lookimg").live("mouseover", function () {
    if ($(this).attr("ISUP") == "1")
        $(this).children(".lookimg_delBtn").eq(0).css("display", "block");;
});
$("#div_imglook .lookimg").live("mouseout", function () {
...
$("#btn_ImgUpStart").click(function () {
    ...
    if ($("#div_imglook .lookimg").length <= 0) {
        alert("还未选择需要上传的图片");
        return;
    }

    //全部图片上传完毕限制
    if ($(".lookimg[ISUP=1]").length == $("#div_imglook .lookimg").length) {
        alert("图片已全部上传完毕！");
        return;
    }
    ...
    for (var i = 0; i < $("#div_imglook .lookimg").length; i++) {
        var NOWLOOK = $("#div_imglook .lookimg").eq(i);//当前操作的图片预览对象
        NOWLOOK.index = i;
        //如果当前图片已经上传，则不再重复上传
        ...
        $.ajax({
            type: "POST",
            url: IMG_AJAXPATH,
            data: { 'imgBase': IMG_BASE, 'imgFormat': IMG_FOMATE, 'lookIndex': NOWLOOK.index, 'csrfmiddlewaretoken': csrfmiddlewaretoken },//图片base64编码，图片格式（当前仅支持jpg,png,jpeg三种），图片对象索引
            dataType: "json",
            success: function (data) {
                if (data.status) {
                    //图片上传成功回调
                    var UPTIME = Math.ceil(Math.random() * 400) + 400;//生成一个400-800的随机数，假设进图条加载时间不一致
                    $("#div_imglook .lookimg").eq([data.ind]).attr("ISUP", "1");//记录此图片已经上传
                    $("#div_imglook .lookimg").eq([data.ind]).children("img").eq(0).attr("src", data.img_url);//记录此图片已经上传
                    $("#div_imglook .lookimg").eq([data.ind]).children(".lookimg_progress").eq(0).children("div").eq(0).animate({ width: "100%" }, UPTIME, function () {
                        $(this).css("background-color", "#00FF00").text('上传成功');
                        setTimeout(function () {
                            $("#div_imglook .lookimg").eq([data.ind]).children(".lookimg_progress").eq(0).remove();
                            // $("#div_imglook .lookimg").eq([data.ind]).children("img").eq(0).attr("onclick", "window.open('" + data.img_url + "')");//记录此图片已经上传
                        }, 1000);
                    });
                }
                else {//图片未上传成功回调
