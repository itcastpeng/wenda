                    ...
                    if zhidao_url.startswith('https://zhidao.baidu') or zhidao_url.startswith('http://zhidao.baidu'):
                        ...
                        if str_ret['status']:
                            ...
                            if daan_str[0] in zhidao_daan:
                                ...
                                self.browser.get_screenshot_as_file('./picture/' + keyword + '--1--' + '{guanjianci_num}.png'.format(guanjianci_num=guanjianci_num))
                                print('---下拉--标红截图第二张---')
                                js = """$("div[order='%d']").css({"border":"3px solid red"})""" % int(order)
                                sleep(self.rand)
                                self.browser.execute_script(js)
                                sleep(self.rand)
                                self.browser.get_screenshot_as_file('./picture/' + keyword + '--2--' + '{guanjianci_num}.png'.format(guanjianci_num=guanjianci_num))
                                self.browser.get(zhidao_url)
                                print('---下拉--截第三张截图---')
                                js = """$(".best-answer-container").css({"border":"3px solid red"})"""
                                self.browser.execute_script(js)
                                sleep(self.rand)
                                self.browser.get_screenshot_as_file('./picture/' + keyword + '--3--' + '{guanjianci_num}.png'.format(guanjianci_num=guanjianci_num))
                                sleep(3)

                                jieping_url = "http://127.0.0.1:8006/api/fifty_guanjianci_fabu"
                                jieping_1 = open('./picture/' + keyword + '--1--' + '{guanjianci_num}.png'.format(guanjianci_num=guanjianci_num),'rb').read()

                                jieping_2 = open('./picture/' + keyword + '--2--' + '{guanjianci_num}.png'.format(guanjianci_num=guanjianci_num),'rb').read()

                                jieping_3 = open('./picture/' + keyword + '--3--' + '{guanjianci_num}.png'.format(guanjianci_num=guanjianci_num),'rb').read()
